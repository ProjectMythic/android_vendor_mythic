...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/MythicOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.mythic.device | grep -q "$MYTHIC_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $MYTHIC_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function repopick() {
    T=$(gettop)
    $T/vendor/mythic/build/tools/repopick.py $@
}
