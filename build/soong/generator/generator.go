...
func init() {
	android.RegisterModuleType("mythic_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = mythicExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = mythicExpandVariables(ctx, dep_file)
	...
	cmd := mythicExpandVariables(ctx, String(g.properties.Cmd))
